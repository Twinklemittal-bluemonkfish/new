...
function nasa_enqueue_styles_libs() {
     ...
     */
    if (NASA_WOO_ACTIVED) {
        wp_enqueue_style('nasa-sc-woo', NASA_CORE_PLUGIN_URL . 'assets/css/nasa-sc-woo.css');
    }
    
    /**
     * Nasa Shortcodes
     */
    wp_enqueue_style('nasa-sc', NASA_CORE_PLUGIN_URL . 'assets/css/nasa-sc.css');
}
...
function nasa_core_scripts_libs() {
     ...
     */
    if (NASA_WOO_ACTIVED) {
        wp_register_script('select2', WC()->plugin_url() . '/assets/js/select2/select2.full.min.js', array('jquery'), null, true);
